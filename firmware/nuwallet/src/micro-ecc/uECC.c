...
/* NuWallet 패치: Arduino 환경에는 nRF5 SDK 헤더(app_error.h, nrf_crypto_rng.h)가
 * 없어서 platform-specific.h 의 NRF52 기본 RNG 가 컴파일되지 않는다.
 * 기본 RNG 를 끄고, setup() 에서 CC310 TRNG 를 uECC_set_rng() 로 넣는다.
 * (RNG 는 서명 자체에는 필요 없다 — RFC6979 결정론적 k 를 쓴다. 스칼라 곱셈의
 *  부채널 블라인딩에만 쓰인다.) */
#ifndef uECC_NO_DEFAULT_RNG
...
#if (uECC_PLATFORM == uECC_avr)
    #include "asm_avr.h"   /* NuWallet: AVR 파일은 제거됨 (ARM 전용) */
#endif
...
/* ===== NuWallet 패치 =========================================================
 * Ethereum 서명에는 recovery id(v) 가 필요하다. 그 재료인 R = k*G 의 y 좌표는
 * uECC_sign_with_k_internal 안에서 이미 계산되어 p 에 들어 있다. 밖으로
 * 내보내기만 하면 된다. 암호 연산 자체는 한 줄도 바꾸지 않았다.
 * (trezor-crypto 의 ecdsa_sign_digest 가 pby 를 돌려주는 것과 같은 방식)
 *
 * 변경 요약
 *   1. uECC_sign_with_k_internal 에 `int *recovery_id` 출력 인자 추가
 *   2. EccPoint_mult 직후 y 패리티(+ x>=n 여부)를 기록
 *   3. 기존 호출자 2곳은 0 을 넘겨 동작이 그대로다
 *   4. uECC_sign_deterministic_recoverable() 추가, 원래 이름은 래퍼로
 *
 * 원본: https://github.com/kmackay/micro-ecc  (BSD-2-Clause, LICENSE.txt 동봉)
 * ========================================================================== */
static int uECC_sign_with_k_internal(const uint8_t *private_key,
    ...
    /* NuWallet 패치: R 의 y 패리티 = recovery id bit0.
     * bit1 은 r = x mod n 에서 실제 감산이 일어난 경우. secp256k1 은 n 이 p 에
     * 매우 가까워 확률 2^-128 수준이라 실무상 0 이지만 정확히 계산해 둔다.
     * p[num_words] 는 y 의 최하위 워드다. */
    if (recovery_id) {
        *recovery_id = (int)(p[num_words] & 1);
        if (uECC_vli_cmp_unsafe(curve->n, p, num_words) != 1) {
            *recovery_id |= 2;
        }
    }
...
int uECC_sign_with_k(const uint8_t *private_key,
                            ...
                            uint8_t *signature,
                            uECC_Curve curve) {
    return uECC_sign_with_k_recoverable(private_key, message_hash, hash_size, k,
                                        signature, curve, 0);
}

/* NuWallet 패치: 외부에서 만든 k 로 서명하고 recovery id 를 돌려준다.
 * uECC 자체 결정론적 서명은 RFC6979 와 다른 k 를 만들기 때문에, 표준을 따르려면
 * k 를 밖에서 계산해 넣어야 한다 (crypto/rfc6979.c). */
int uECC_sign_with_k_recoverable(const uint8_t *private_key,
                            const uint8_t *message_hash,
                            unsigned hash_size,
                            const uint8_t *k,
                            uint8_t *signature,
...
int uECC_sign(const uint8_t *private_key,
    ...
    for (tries = 0; tries < uECC_RNG_MAX_TRIES; ++tries) {
        ...
        if (uECC_sign_with_k_internal(private_key, message_hash, hash_size, k, signature, curve, 0)) {
            return 1;
        }
    }
    return 0;
...
int uECC_sign_deterministic_recoverable(const uint8_t *private_key,
    ...
    filled:
        ...
        if (uECC_sign_with_k_internal(private_key, message_hash, hash_size, T, signature, curve, recovery_id)) {
            return 1;
        }
...
int uECC_sign_deterministic(const uint8_t *private_key,
                            ...
                            uint8_t *signature,
                            uECC_Curve curve) {
    return uECC_sign_deterministic_recoverable(private_key, message_hash, hash_size,
