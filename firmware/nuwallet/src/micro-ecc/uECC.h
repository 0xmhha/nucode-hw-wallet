...
Returns 1 if the signature generated successfully, 0 if an error occurred.
*/
/* NuWallet 패치: nRF5 SDK 가 없는 Arduino 빌드에서는 기본 RNG 를 쓰지 않는다.
   CC310 TRNG 를 uECC_set_rng() 로 주입한다. */
#ifndef uECC_NO_DEFAULT_RNG
#define uECC_NO_DEFAULT_RNG 1
#endif

/* NuWallet 패치: 외부 k 로 서명 + recovery id.
   표준 RFC6979 k 를 crypto/rfc6979.c 에서 만들어 넣는다. */
...
/* NuWallet 패치: RFC6979 결정론적 서명 + Ethereum recovery id.
   recovery_id 에 0..3 이 들어간다. Ethereum 의 v = recovery_id + 27,
   EIP-155 에서는 recovery_id + chainId*2 + 35. */
