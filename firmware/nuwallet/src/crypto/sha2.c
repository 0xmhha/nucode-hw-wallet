...
void sha256_update(sha256_ctx *c, const uint8_t *d, size_t n) {
    c->len += n;
    while (n) {
        size_t k = SHA256_BLOCK - c->n; if (k > n) k = n;
        memcpy(c->buf + c->n, d, k); c->n += k; d += k; n -= k;
        if (c->n == SHA256_BLOCK) { sha256_block(c, c->buf); c->n = 0; }
    }
}
...
#define T0(x) (ROR64(x,28)^ROR64(x,34)^ROR64(x,39))
#define T1(x) (ROR64(x,14)^ROR64(x,18)^ROR64(x,41))
#define t0(x) (ROR64(x,1)^ROR64(x,8)^((x)>>7))
#define t1(x) (ROR64(x,19)^ROR64(x,61)^((x)>>6))

static void sha512_block(sha512_ctx *c, const uint8_t *p) {
    uint64_t w[80], a,b,cc,d,e,f,g,h,x1,x2; int i;
    for (i = 0; i < 16; i++) {
        w[i] = 0;
        for (int j = 0; j < 8; j++) w[i] = (w[i] << 8) | p[i*8+j];
    }
    for (; i < 80; i++) w[i] = t1(w[i-2]) + w[i-7] + t0(w[i-15]) + w[i-16];
    a=c->h[0];b=c->h[1];cc=c->h[2];d=c->h[3];e=c->h[4];f=c->h[5];g=c->h[6];h=c->h[7];
    for (i = 0; i < 80; i++) {
        x1 = h + T1(e) + CH(e,f,g) + K512[i] + w[i];
        x2 = T0(a) + MAJ(a,b,cc);
        h=g; g=f; f=e; e=d+x1; d=cc; cc=b; b=a; a=x1+x2;
    }
    c->h[0]+=a;c->h[1]+=b;c->h[2]+=cc;c->h[3]+=d;c->h[4]+=e;c->h[5]+=f;c->h[6]+=g;c->h[7]+=h;
}
...
void sha512_update(sha512_ctx *c, const uint8_t *d, size_t n) {
    c->len += n;
    while (n) {
        size_t k = SHA512_BLOCK - c->n; if (k > n) k = n;
        memcpy(c->buf + c->n, d, k); c->n += k; d += k; n -= k;
        if (c->n == SHA512_BLOCK) { sha512_block(c, c->buf); c->n = 0; }
    }
}
void sha512_final(sha512_ctx *c, uint8_t out[64]) {
    ...
    /* 길이는 128비트지만 상위 64비트는 항상 0 (2^61 바이트 미만) */
    for (i = 0; i < 8; i++) sha512_update(c, &pad, 1);
    for (i = 7; i >= 0; i--) { uint8_t b = (uint8_t)(bits >> (i*8)); sha512_update(c, &b, 1); }
    for (i = 0; i < 8; i++)
        for (int j = 0; j < 8; j++) out[i*8+j] = (uint8_t)(c->h[i] >> (56 - j*8));
}
