...
기기는 랜덤 챌린지를 띄우고, 사용자가 **기기 버튼으로 승인**해야 실제로 바뀐다.
...
- 5회 틀리면 `0x6502 CHALLENGE_FAILED` 를 내고 재부팅 전까지 잠긴다.
...
`SIGN_SOLANA` 성공 이벤트는 `REQUEST_ID(4) | STATUS(2) | SIGNATURE(64)`로
총 70바이트이며 recovery id가 없다.
