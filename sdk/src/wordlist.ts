 ...
 *
 * 기기는 단어 문자열이 아니라 **인덱스**를 주고받는다 (docs/protocol.md §5).
 * BLE 대역폭을 아끼려는 것도 있지만, 더 큰 이유는 기기가 문자열 파싱을 하지
 * 않아도 되기 때문이다 — 파서는 공격면이다.
 *
 * 출처: https://github.com/trezor/python-mnemonic  english.txt
...
/** 인덱스 -> 단어. 범위를 벗어나면 던진다. */
export function indexToWord(i: number): string {
  const w = WORDLIST[i];
  if (w === undefined) throw new WalletError(SW.BAD_PARAM, `워드 인덱스 범위 밖: ${i}`);
  return w;
}

/**
 * 단어 -> 인덱스. BIP-39 단어는 앞 4글자로 유일하게 구분되므로,
 * 사용자가 4글자만 쳐도 받아 준다.
 */
export function wordToIndex(word: string): number {
  ...
  if (w.length >= 4) {
    ...
    for (let i = 0; i < WORDLIST.length; i++) {
      if (WORDLIST[i]!.startsWith(w)) hits.push(i);
    }
    if (hits.length === 1) return hits[0]!;
  }
  throw new WalletError(SW.BAD_PARAM, `워드리스트에 없는 단어: ${word}`);
}

/** 니모닉 문자열 -> 인덱스 배열. 길이가 12/15/18/21/24 가 아니면 던진다. */
export function mnemonicToIndices(mnemonic: string): number[] {
  ...
  if (![12, 15, 18, 21, 24].includes(words.length)) {
    throw new WalletError(SW.BAD_PARAM,
      `니모닉은 12/15/18/21/24 단어여야 합니다 (지금 ${words.length}개)`);
  }
  return words.map(wordToIndex);
}

/** 인덱스 배열 -> 니모닉 문자열. */
export function indicesToMnemonic(indices: number[]): string {
  return indices.map(indexToWord).join(' ');
}
